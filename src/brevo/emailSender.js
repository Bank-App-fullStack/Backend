...
module.exports = function senderEmail(user) {
  try{
    ...
    sendSmtpEmail.subject = "New user is registered";
    sendSmtpEmail.sender = { "email": `${user.mail}`, "name": `${user.firstname} ${user.lastname}`};
    // sendSmtpEmail.to = [{ "email": "flobertjohnson@gmail.com", "name":"John Doe"}];
    sendSmtpEmail.to = [{"name":"John Doe","email":"flobertjohnson@gmail.com"}]; // , {"name": `${user.firstname} ${user.lastname}`, "email": `${user.mail}`}
